...
// sim pin
#define GSM_PIN "redacted"

// gprs credentials for digitec iot
const char apn[] = "redacted";
const char gprsUser[] = "redacted";
const char gprsPass[] = "redacted";

// mqtt broker
const char *broker = "redacted";
...
boolean mqttConnect()
{
  ...
  boolean status = mqtt.connect("GsmClientName", mqttUser, mqttPassword, "redacted", 0, 1, "Scale Timeout");
  ...
  mqtt.publish("redacted", "Scale Connected", true);
...
void setup()
{
  ...
  // publish measurement
  mqtt.publish("redacted", ((String)temp).c_str(), true);
  mqtt.publish("redacted", ((String)wgt).c_str(), true);
  ...
  mqtt.publish("redacted", "Scale Disconnected", true);
